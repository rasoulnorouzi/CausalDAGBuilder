...
if __name__ == "__main__":
    ...
    # print unique labels for each annotator
    for processor in processors:
        annotations = processor.get_annotations()
        labels = [annotation["labels"] for annotation in annotations]
        unique_labels = set(label for labels in labels for label in labels)
        print(unique_labels)

    # Create a FuzzyKrippendorff instance (considering both "cause" and "effect" targets).
    fk = FuzzyKrippendorff(annotation_processors=processors, targets=["cause", "effect", "cause+effect"])
